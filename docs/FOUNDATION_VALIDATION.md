...
The F08 cross-repository fixture provisions and durably installs identity, freezes a Primitive directory from copied Serializable maxima and an opaque static descriptor, queries all format bounds, and round-trips a bounded payload with heap allocation forbidden throughout. This test-only composition does not add a production dependency edge. It runs in the Serializable integration CI job.

T1 dependent migration: the optional logging worker now uses split TaskExecutorConfiguration, an explicit one-token inline FIFO and a fixed noexcept member thunk. Its compile contract was reclassified to test the new execution/backlog boundary. The obsolete combined TaskConfiguration and capturing handler are removed.
